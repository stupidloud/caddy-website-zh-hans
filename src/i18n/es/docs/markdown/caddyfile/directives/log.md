---
title: log (Directiva de Caddyfile)
---
...
Habilita y configura el registro de solicitudes HTTP (también conocido como logs de acceso).

<aside class="tip">

Para configurar los logs en tiempo de ejecución de Caddy, consulta la [`log` opción global](/docs/caddyfile/options#log) en su lugar.

</aside>


La directiva `log` se aplica a los hostnames del bloque de sitio donde aparece, salvo que se sobrescriba con la subdirectiva `hostnames`.

Cuando está configurada, por defecto se registrarán todas las solicitudes al sitio. Para omitir condicionalmente algunas solicitudes del registro, usa la directiva [`log_skip`](log_skip).

Para agregar campos personalizados a las entradas del log, usa la directiva [`log_append`](log_append).


- [Sintaxis](#syntax)
- [Módulos de salida](#output-modules)
  - [stderr](#stderr)
  - [stdout](#stdout)
  - [discard](#discard)
  - [file](#file)
...
Por defecto, cabeceras con información potencialmente sensible (`Cookie`, `Set-Cookie`, `Authorization` y `Proxy-Authorization`) se registrarán como `REDACTED` en los access logs. Este comportamiento se puede desactivar con la opción global de servidor [`log_credentials`](/docs/caddyfile/options#log-credentials).
...
- **logger_name** <span id="logger_name"/> es un sobrescritura opcional del nombre del logger para este sitio.

  Por defecto, el nombre de logger se genera automáticamente, por ejemplo `log0`, `log1`, etc., según el orden de los sitios en el Caddyfile. Esto solo es útil si quieres hacer referencia de forma confiable a la salida de este logger desde otro logger definido en las opciones globales. Ver [un ejemplo](#multiple-outputs) más abajo.

- **hostnames** <span id="hostnames"/> es una sobrescritura opcional de los hostnames a los que se aplica este logger.

  Por defecto, el logger se aplica a los hostnames del bloque de sitio donde aparece, es decir, las direcciones del sitio. Esto es útil si quieres definir diferentes loggers por subdominio en un [bloque wildcard](/docs/caddyfile/patterns#wildcard-certificates). Ver [un ejemplo](#wildcard-logs) más abajo.

- **no_hostname** <span id="no_hostname"/> evita que el logger se asocie con ninguno de los hostnames del bloque del sitio. Por defecto, el logger se asocia con la [dirección del sitio](/docs/caddyfile/concepts#addresses) donde aparece la directiva `log`.

  Esto es útil cuando quieres registrar solicitudes en archivos diferentes según alguna condición, como la ruta o método de la solicitud, usando la directiva [`log_name`](/docs/caddyfile/directives/log_name).

- **output** <span id="output"/> configura dónde escribir los logs. Ver [módulos `output`](#output-modules) abajo.

  Valor predeterminado: `stderr`.

- **format** <span id="format"/> describe cómo codificar o formatear los logs. Ver [módulos `format`](#format-modules) abajo.

  Valor predeterminado: `console` si se detecta que `stderr` es una terminal, `json` en otro caso.

- **level** <span id="level"/> es el nivel mínimo de entrada para registrar. Valor predeterminado: `INFO`.

  Ten en cuenta que actualmente los access logs solo emiten niveles `INFO` y `ERROR`.

- **sampling** <span id="sampling"/> configura el muestreo de logs para reducir el volumen. Si se especifica `sampling`, se habilita con los valores predeterminados que aparecen abajo. Omitirlo desactiva el muestreo.

  - **interval** es la [ventana de duración](/docs/conventions#durations) en la que se hace el muestreo. Predeterminado: `1s` (deshabilitado).

  - **first** es cuántos logs conservar dentro de un nivel y mensaje dado para cada intervalo. Predeterminado: `100`.

  - **thereafter** es cuántos logs saltar en cada intervalo después de los primeros conservados. Predeterminado: `100`.

  Por ejemplo, con `interval 1s`, `first 5` y `thereafter 10`, en cada intervalo de 10 segundos se conservarán las primeras 5 entradas y luego pasará 1 de cada 10 con el mismo nivel y mensaje dentro de ese segundo.


### Output modules

La subdirectiva **output** te permite personalizar dónde se escriben los logs.

#### stderr

Error estándar (consola, es el valor predeterminado).
...
Un archivo. Por defecto, los archivos de log se rotan ("rolled") según el tamaño para evitar agotamiento de espacio en disco.

La rotación de logs la proporciona [timberjack <img src="/old/resources/images/external-link.svg" class="external-link">](https://github.com/DeRuina/timberjack)

<aside class="tip">

**Nota sobre recargar opciones de archivo de log:** se requiere reiniciar el servidor para aplicar cambios de configuración a un archivo de salida concreto.
Los cambios no se aplican al recargar el servidor, salvo que añadas un nombre nuevo de archivo de log.
...
- **&lt;filename&rt;** is the path to the log file.
