...
- **replace_status** <span id="replace_status"/> 仅在匹配到给定的匹配器时更改响应的状态码。

- **handle_response** <span id="handle_response"/> 用于定义当原始响应与给定的响应匹配器匹配时应执行的路由。如果省略匹配器，则所有响应都会被拦截。当定义了多个 `handle_response` 块时，将应用第一个匹配的块。在该块内部，可以使用所有其他[指令](/docs/caddyfile/directives)。
