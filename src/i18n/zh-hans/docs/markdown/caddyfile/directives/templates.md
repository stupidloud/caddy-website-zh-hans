...
模板可能会在来自*任何*来源的响应正文上执行，无论它是磁盘上的静态文件，还是被代理的 Web 服务。明智的做法是只对你信任、可控和/或已净化的内容启用模板求值！配置不当可能导致安全漏洞。例如，如果一个被代理的应用允许用户撰写/发布内容，而这些内容包含看起来像模板操作的文本，就会让任意用户得以执行模板，并可能借此访问环境变量、本地文件和网络。不要在用户生成的内容上启用模板（除非已先净化）。
