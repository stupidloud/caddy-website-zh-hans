...
템플릿은 디스크의 정적 파일이든 프록시된 웹 서비스이든 *모든* 출처의 응답 본문에 대해 실행될 수 있습니다. 템플릿 평가는 신뢰할 수 있고, 제어할 수 있으며, 그리고/또는 새니타이즈(sanitize)한 콘텐츠에 대해서만 활성화하는 것이 현명합니다! 잘못된 설정은 보안 침해로 이어질 수 있습니다. 예를 들어 프록시된 앱이 사용자에게 콘텐츠 작성/게시를 허용하고, 그 콘텐츠에 템플릿 액션처럼 보이는 텍스트가 포함되어 있다면, 임의의 사용자가 템플릿을 평가하여 환경 변수, 로컬 파일, 네트워크에 접근할 수 있게 됩니다. 사용자 생성 콘텐츠에 대해서는 (새니타이즈하지 않은 채) 템플릿을 활성화하지 마세요.
